    ...
    topicOpen.addEventListener('click', (e) => {
        ...
        function mostrar_oldPosts(topicClassName) {
            ...
            for (let i = parseInt(topicID); i >= 0; i--) {
                ...
                if (listPost !== null && listPost !== undefined) {
                    ...
                    // Condicional para que cada user solo pueda eliminar sus posts
                    let extractName = listPost.split(' ')[0]; 
                    let extractUser = sessionStorage.getItem('userName');
                    wrapper_oldPost.appendChild(div_oldPost);
                    console.log(extractUser)
                    if (extractName == extractUser) {
        ...
        send_newPost.addEventListener('click', (e) => {
            ...
            if (post_text.length > 1) {
                let post_newTextarea = `${sessionStorage.getItem('userName')} ha posteado el dia ${new Date().toUTCString()}: ${post_text}`;
