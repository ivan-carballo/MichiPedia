...
async function catsFact() {
    ...
    try {
        ...
        // API de traduccion para que la info del gato este en castellano y no en ingles
        traducir(fact);
        async function traducir(fact) {
            const url = new URL(`https://api.mymemory.translated.net/get?`);
            url.searchParams.append("q", fact);
            url.searchParams.append('langpair', 'en|es')
            try {
                const response = await fetch(url.toString());
                let data = await response.json();
                divFact.innerText = data.responseData['translatedText'];
            }
            catch (error) {
                console.error(error);
            }
